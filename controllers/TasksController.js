...
exports.tasksController = {
    add:  async(req, res) => {
        try{
            ...
            if(!klass && !body && !header && subject) return res.status(400).json({message: 'no data'})
            const post = new TasksSchema({
                klass, header, body, img, subject, date
            })
    ...
    get: async(req, res) => {
        try{
            ...
            const task = await TasksSchema.find({klass, subject}).limit(limit*1).skip((page-1)*limit)
    ...
    get_with_filter: async(req, res) => {
        try{
            ...
            const result = task.filter((e) => e.header.slice(0, filter.length).toLowerCase() == filter.toLowerCase())
